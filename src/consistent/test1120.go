...
func JumpConsistentHash(key uint64, num_buckets int) int {
	var b, j int64
	b = -1
	for int(j) < num_buckets {
		b = j
...
func main() {
	...
	host, _ := consistent.Get("aa")
	fmt.Println("host:", host)

	host, _ = consistent.Get("bb")
	fmt.Println("host:", host)

	host, _ = consistent.Get("cc")
	fmt.Println("host:", host)

	//consistent.remove("127.0.0.3")
